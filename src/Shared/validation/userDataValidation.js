...
.test('FILE_SIZE', 'Uploaded file is too large', (value) =>
  ...
  .test('fileType', 'Unsupported file format', (value) => !value || (value && SUPPORTED_FORMATS.includes(value?.type))),
    ...
    // .required('Required')
    ,
    birthday: Yup.string().matches(/^\d{2}\.\d{2}\.\d{4}$/, 'Birthday must be in format DD.MM.YYYY')
    // .required('Required')
    ,
    phone: Yup.string().matches(/^\+380\d{9}$/, 'Phone number must be in format +380000000000')
    // .required('Required')
    ,
